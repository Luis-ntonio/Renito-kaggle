...
class PerplexityCalculator:
    ...
    def sort_words(self, sentence):
        print(sentence)
        model_inputs = self.tokenizer(
                    f"If a sentence start with 'mistletoe' which will be the best order to concatenate the words of the array? {[s for s in sentence.split(' ') if s != 'mistletoe']} return only the array",
                    return_tensors='pt',
                )
        model_inputs = {k: v.to(DEVICE) for k, v in model_inputs.items()}
        print("Model inputs")
        outputs = self.model.generate(**model_inputs, max_new_tokens=130)
